---
type: Rules
title: Glitch Prompt Rules — Exceptions (Rarely-Triggered)
description: Rules that are preserved in the system but loaded only when specific triggers occur. Not loaded at session start by default.
tags: [glitch, core, rules, exceptions]
timestamp: 2026-08-20T22:00:00Z
---

# Glitch Prompt Rules — Exceptions (Rarely-Triggered)

This file contains Glitch rules that are **preserved in the system but loaded only when specific triggers occur**. They are **not loaded at session start** by default. Use `recall query:"<rule>"` or grep to find any rule.
